...
function HedgeTabs({ block }: { block: Extract<Block, { type: 'hedgeTabs' }> }) {
  ...
  return (
    <div>
      ...
      <div className="mt-4">
        {/* Hedging wording preserved here */}
        <BarLine
...
/* ----------------------------- Page ----------------------------- */
export default function FinancialsQ2Page() {
  const [doc, setDoc] = useState<BlocksDoc | null>(null);

  useEffect(() => {
    let alive = true;
    fetch('/data/briefings/q2-2025.blocks.json', { cache: 'no-store' })
    ...
    <div className="mx-auto max-w-6xl px-4 py-10">
      ...
      {!doc ? (
        <div className="rounded-2xl border p-6 text-sm" style={{ borderColor: BRAND.border, color: BRAND.subtext }}>
          Couldn’t load blocks JSON. Check <code>/public/data/briefings/q2-2025.blocks.json</code>.
        </div>
      ) : (
        doc.slides.map((s) => (
